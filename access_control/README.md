### Run access_control examples (storing and retrieving permissioned secrets, revoking permissions)
...
2. The writer stores a secret and gives the reader retrieve access_control on the secret based on the reader's user id, resulting in a store id for the secret
3. The reader retrieves the secret with the store id
4. The writer revokes secret access_control by rewriting them
